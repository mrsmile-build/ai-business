...
- **Lead Finder rescue engine / HasData fallback (verified on localhost with fake HasData keys).** When HasData fails or runs out of credits, /api/lead-finder calls our own Search API (private repo ai-business-search-api, 2 Render mirrors) which geocodes city+country via Nominatim (cached), radius-searches OpenStreetMap (best mirror first, 45s timeout), caches results into shared Supabase businesses table. Country-scoped so Lagos Nigeria never returns Lagos Portugal; wrong-country results impossible; No-website filter honored in fallback. Verified: with all HasData keys faked, Lead Finder returned 15 real Lagos businesses (Mama Cass, Mr. Bigs, etc.) tagged Online. Groq personalization skipped in rescue mode (simple template) by design. Contact data (phone/website) sparse in OSM Nigeria — rescue mode delivers leads but not full contact info; HasData remains primary for quality.
- **Lead Finder opportunity radar (v1.7.1, SHIPPED).** Search API now scores each lead 0-10 based on website age (WHOIS), contact completeness, OSM footprint. UI shows 🔥 badge for scores ≥7. Fallback messages personalized: 'doesn't have a website yet' pitch for high-scoring leads. Multi-category rescue (restaurant,fast_food,cafe) for sparse categories. 60s timeout cap prevents UI hangs. GitHub Actions keep-warm pings mirrors every 14 minutes.
